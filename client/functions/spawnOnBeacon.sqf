...
if (KEEPWEAPON && !isNil"OLDWEAPONS")then{
	{ 
		player addWeapon _x;
...
_hour = date select 3;
_mins = date select 4;
["Chocoland", format ["%1'%2 Parachute Spawn", name player, ""], format ["%1:%3%2", _hour, _mins, if (_mins < 10) then {"0"} else {""}]] spawn BIS_fnc_infoText;
