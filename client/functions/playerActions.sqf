...
[
	...
	[format ["<img image='client\icons\r3f_unlock.paa'/> [Attach this Vehicle]", "#0101DF"], "client\choco\attachThisVehicle.sqf", [cursorTarget], -8, true, true, "", "!ATTACHING && alive cursorTarget && cursorTarget isKindOf 'AllVehicles' && {player distance cursorTarget <= (sizeOf typeOf cursorTarget / 3) max 3}"],
	
	[format ["<img image='client\icons\r3f_unlock.paa'/> [Attach to this Vehicle]", "#0101DF"], "client\choco\attachThisVToThis.sqf", [], -7, true, true, "", "ATTACHING && alive cursorTarget && {cursorTarget isKindOf 'AllVehicles' && player distance cursorTarget <= (sizeOf typeOf cursorTarget / 3) max 3}"],
	
	[format ["<img image='\a3\ui_f\data\map\vehicleicons\iconanimal_ca.paa'/> [Catch animal]", "#0101DF"], "client\choco\catchAnimal.sqf",[], -8, true, true, "", "count (_name = nearestobjects [_target, call _animalArray,  5]) != 0],
	...
	[format ["<img image='\a3\ui_f\data\map\vehicleicons\iconanimal_ca.paa'/> [Plant a Bomb (G)]", "#0101DF"], "client\choco\animalPlantAction.sqf", [], -7, true, true, "", "ANIAMLBOMB && ANIMALMODE"],
	...
	[format ["<img image='\A3\ui_f\data\igui\cfg\simpletasks\types\container_ca.paa'/> [claim basecore reward]", "#0101DF"], "client\choco\fnc_destroyBasecore.sqf", [], -6, true, true, "", "typeOf cursorTarget == 'Land_Device_assembled_F' && {player distance cursorTarget <= (sizeOf typeOf cursorTarget / 3) max 3}"],
