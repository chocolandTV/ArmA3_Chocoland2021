...
class gunshopd
{
	...
	class Controls
	{
		...
		class BuyGun2 : w_RscButton
		{
			idc = -1;
			action = "closeDialog 0;[] execVM 'client\systems\generalStore\loadGenStore.sqf'";
			text = "Generals";
			sizeEx = GunStoreButton_textSize * 2;

			x = GunStoreMainBG_X + (0.100 * SZ_SCALE);
			y = GunStoreMainBG_Y - (0.107 * SZ_SCALE);
			w = 0.096 * SZ_SCALE * 2;
			h = 0.040 * SZ_SCALE * 2;
		};
		class BuyGun3 : w_RscButton
		{
			idc = -1;
			action = "closeDialog 0;[] execVM 'client\systems\vehicleStore\loadVehicleStore.sqf'";
			text = "Vehicles";
			sizeEx = GunStoreButton_textSize * 2;

			x = GunStoreMainBG_X + (0.434 * SZ_SCALE);
			y = GunStoreMainBG_Y - (0.107 * SZ_SCALE);
			w = 0.096 * SZ_SCALE * 2;
			h = 0.040 * SZ_SCALE * 2;
		};
