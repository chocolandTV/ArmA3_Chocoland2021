...
class genstored
{
	...
	class Controls
	{
		...
		class BuyItem2 : w_RscButton
		{
			idc = -1;
			action = "closeDialog 0;[] execVM 'client\systems\gunStore\loadGunStore.sqf'";
			text = "Weapons";
			sizeEx = GenStoreButton_textSize*2;

			x = GenStoreMainBG_X + (0.100 * SZ_SCALE);
			y = GenStoreMainBG_Y - (0.107 * SZ_SCALE);
			w = 0.096 * SZ_SCALE*2;
			h = 0.040 * SZ_SCALE*2;
		};
		class BuyItem3 : w_RscButton
		{
			idc = -1;
