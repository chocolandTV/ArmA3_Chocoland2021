...
class vehshopd
{
	...
	class Controls
	{
		...
		class BuyVehicle2 : w_RscButton
		{
			idc = -1;
			action = "closeDialog 0;[] execVM 'client\systems\generalStore\loadGenStore.sqf'";
			text = "Generals";
			sizeEx = VehStoreButton_textSize * 2;

			x = VehStoreMainBG_X + (0.100 * SZ_SCALE);
			y = VehStoreMainBG_Y - (0.107 * SZ_SCALE);
			w = 0.096 * SZ_SCALE * 2;
			h = 0.040 * SZ_SCALE * 2;
		};
		class BuyVehicle3 : w_RscButton
		{
			idc = -1;
			action = "closeDialog 0;[] execVM 'client\systems\gunStore\loadGunStore.sqf'";
			text = "Weapons";
			sizeEx = VehStoreButton_textSize * 2;

			x = VehStoreMainBG_X + (0.434 * SZ_SCALE);
			y = VehStoreMainBG_Y - (0.107 * SZ_SCALE);
			w = 0.096 * SZ_SCALE * 2;
			h = 0.040 * SZ_SCALE * 2;
		};
